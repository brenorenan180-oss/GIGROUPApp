...
(function () {
  ...
  function tryHandle(el) {
    ...
    if (!isTargetGroupExtended()) return;
  ...
  function fb5ScanVisible() {
    if (!enabled || !isTargetGroupExtended()) return;
    var now = Date.now();
  ...
  (function installFB10Interceptors() {
    ...
    window.fetch = function() {
      ...
      return originalFetch.apply(this, args).then(function(response) {
        if (!enabled || !isTargetGroupExtended()) return response;
  ...
  // ── NOVA FUNÇÃO — retorna o nome do grupo atual visível no header ──────────
  ...
  // ── NOVA FUNÇÃO — estendida, verifica grupos adicionais fixos ─────────────
  // Chama a original isTargetGroup() primeiro e depois verifica grupos extras.
  // NÃO altera isTargetGroup() — apenas adiciona detecção.
  var ADDITIONAL_GROUPS = [
    "Gigroup - SHOPEE Juazeiro/BA"
  ];

  function isTargetGroupExtended() {
    // Primeiro mantém o comportamento original intacto
    if (isTargetGroup()) return true;

    // Depois verifica grupos adicionais fixos
    var groupName = getCurrentGroupName();
    return ADDITIONAL_GROUPS.some(function(g) {
      return groupName.toLowerCase() === g.toLowerCase();
    });
  }
